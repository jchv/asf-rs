...
impl<'a> HeaderExtensionData<'a> {
    ...
    pub fn write<T: Write>(&self, w: &mut T) -> Result<(), Box<dyn std::error::Error>> {
        let extension_data_size: u32 = self
            .extension_objects
            .iter()
            .map(|x| x.size_of())
            .sum::<usize>()
