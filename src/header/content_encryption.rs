...
impl<'a> ContentEncryptionData<'a> {
    ...
    pub fn size_of(&self) -> usize {
        let mut len = 0;
        len += 4;
        len += self.secret_data.len();
        len += 4;
        len += self.protection_type.len();
        len += 4;
        len += self.key_id.len();
        len += 4;
        len += self.license_url.len();
        len
    }
}
...
#[cfg(test)]
mod tests {
    use nom::{error::VerboseError, AsBytes, Slice};
    ...
    const BASIC_CONTENT_ENCRYPTION_BYTES: &'static [u8] = &[
        ...
        ,
    ...
    fn parse_basic_content_encryption() {
        assert_eq!(
            HeaderObject::parse::<VerboseError<_>>(Span::new(BASIC_CONTENT_ENCRYPTION_BYTES))
                .expect("parser error")
                .1,
